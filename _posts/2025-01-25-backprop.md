---
title: A Simplest Python Code for Backpropagation
...
In our case, we have to simply nudge the values in the opposite direction of the gradient to bring the loss down, as all the derivatives are positive and hence influencing loss positively.
...
Please note that, usually the loss is negative, and we try to increase the loss upward towards zero. Hence it will be `+=` instead of `-=`, and we should consider the sign of derivative.
